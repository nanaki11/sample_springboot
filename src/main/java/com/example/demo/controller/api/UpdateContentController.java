...
import com.example.demo.entity.request.UpdateContentRequest;
...
public class UpdateContentController {
	...
	public ApiResponse update(
			@RequestBody UpdateContentRequest form) {
		...
		if(!isExistingContent(contentId, lang)) {
			String detail = MessageFormat.format(ApiResponseUtil.ERROR_DETAIL_CONTENTID_NOT_EXIST, contentId, lang);
			apiError = ApiResponseUtil.setA002001(detail);
			logger.error(ToStringBuilder.reflectionToString(apiError, ToStringStyle.DEFAULT_STYLE));
			return new ApiResponse(apiError);
		}

		try {
			updateContent(contentId, lang, form.getType(), form.getCommunityId(), form.getTitle(), form.getContents(),
	...
	private ApiError validationCheck(String clientId, String contentId) {
		...
		if (ApiResponseUtil.CONTENT_ID_MAXSIZE < contentId.length() || !StringUtils.isNumeric(contentId)) {
			...
			return ApiResponseUtil.setA002001(detail);
		}
	 ...
	 */
	private void updateContent(String contentId, String lang, String type, String communityId,
