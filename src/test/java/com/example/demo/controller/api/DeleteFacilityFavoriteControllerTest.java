...
public class DeleteFacilityFavoriteControllerTest {
	// RestController試験の設定
	@Autowired
	private MockMvc mvc;
	@Autowired
	...
	// 試験対象の設定
	private static final String path = "/api/favorite/facility/delete";
	@InjectMocks
	private DeleteFacilityFavoriteController controller;
	@MockBean
	...
	public void main_success_test1() throws Exception {
		...
		// 想定する返却値の設定
		ApiResponse res = new ApiResponse(new DeleteFacilityFavoriteResponse());
		res.setStatus(0);
		...
		// 削除データのモック処理
		doNothing().when(facilityFavoriteService).delete(any(FacilityFavorite.class));
		...
		verify(facilityFavoriteService, times(1)).delete(any(FacilityFavorite.class));
	}
	...
	public void main_error_test1() throws Exception {
		...
		verify(facilityFavoriteService, times(0)).delete(any(FacilityFavorite.class));
	}
	...
	public void validationCheck_test6()
		...
		assertEquals(true, (res.getId().matches("^[0-9]{18}_[0-9a-zA-Z]{4}$")));
		assertEquals(ApiResponseUtil.ERROR_CODE_F006001, res.getCode());
	...
	public void validationCheck_test10()
		...
		assertEquals(true, (res.getId().matches("^[0-9]{18}_[0-9a-zA-Z]{4}$")));
		assertEquals(ApiResponseUtil.ERROR_CODE_F006002, res.getCode());
	 ...
	 */
	@Test
	public void deleteFacilityFavorite_test()
			throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {

		// 削除データのモック処理
		ArgumentCaptor<FacilityFavorite> captor = ArgumentCaptor.forClass(FacilityFavorite.class);
		doNothing().when(facilityFavoriteService).delete(captor.capture());

		Method method = controller.getClass().getDeclaredMethod("deleteFacilityFavorite", String.class, String.class);
		method.setAccessible(true);
		method.invoke(controller, customerId, businessCd);

		// 削除処理の引数（施設お気に入り情報）が想定通りか確認する
