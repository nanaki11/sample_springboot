...
public class UpdateContentControllerTest {
	...
	// 試験対象の設定
	private static final String path = "/api/content/update";
	@InjectMocks
	private UpdateContentController controller;
	@MockBean
	...
	public void main_success_test1() throws Exception {
		...
		// 更新データのモック処理
		when(contentService.update(any(Content.class))).thenReturn(new Content());
		...
		verify(contentService, times(1)).update(any(Content.class));
	}
	...
	public void main_error_test1() throws Exception {
		...
		verify(contentService, times(0)).update(any(Content.class));
	}
	...
	/**
	 * API実行試験
	 * 条件：異常終了（更新失敗：ParseException）の場合
	 */
	@Test
	...
	public void validationCheck_test6()
		...
		assertEquals(true, (res.getId().matches("^[0-9]{18}_[0-9a-zA-Z]{4}$")));
		assertEquals(ApiResponseUtil.ERROR_CODE_A002001, res.getCode());
	 ...
	 */
	@Test
	public void updateContent_test1()
		...
		when(contentService.update(captor.capture())).thenReturn(new Content());

		Method method = controller.getClass().getDeclaredMethod("updateContent", String.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class, String.class);
		method.setAccessible(true);
		method.invoke(controller, contentId, lang, "コンテンツ種別", "12345", "コンテンツタイトル", "コンテンツ内容", "コンテンツ画像URL", deliveryStartDate, deliveryEndDate);

		// 更新処理の引数（コンテンツ情報）が想定通りか確認する
	 ...
	 */
	@Test
	public void updateContent_test2()
			throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, ParseException {
