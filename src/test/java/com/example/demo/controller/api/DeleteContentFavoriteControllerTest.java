...
public class DeleteContentFavoriteControllerTest {
	...
	// 試験対象の設定
	private static final String path = "/api/favorite/content/delete";
	@InjectMocks
	private DeleteContentFavoriteController controller;
	@MockBean
	...
	public void main_success_test1() throws Exception {
		...
		// 想定する返却値の設定
		ApiResponse res = new ApiResponse(new DeleteContentFavoriteResponse());
		res.setStatus(0);
		...
		// 削除データのモック処理
		doNothing().when(contentFavoriteService).delete(any(ContentFavorite.class));
		...
		verify(contentFavoriteService, times(1)).delete(any(ContentFavorite.class));
	}
	...
	public void main_error_test1() throws Exception {
		...
		verify(contentFavoriteService, times(0)).delete(any(ContentFavorite.class));
	}
	...
	public void validationCheck_test6()
		...
		assertEquals(true, (res.getId().matches("^[0-9]{18}_[0-9a-zA-Z]{4}$")));
		assertEquals(ApiResponseUtil.ERROR_CODE_F003001, res.getCode());
	...
	public void validationCheck_test10()
		...
		assertEquals(true, (res.getId().matches("^[0-9]{18}_[0-9a-zA-Z]{4}$")));
		assertEquals(ApiResponseUtil.ERROR_CODE_F003002, res.getCode());
	 ...
	 */
	@Test
	public void deleteContentFavorite_test()
			throws NoSuchMethodException, SecurityException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {

		// 削除データのモック処理
		ArgumentCaptor<ContentFavorite> captor = ArgumentCaptor.forClass(ContentFavorite.class);
		doNothing().when(contentFavoriteService).delete(captor.capture());

		Method method = controller.getClass().getDeclaredMethod("deleteContentFavorite", String.class, String.class);
		method.setAccessible(true);
		method.invoke(controller, customerId, contentId);

		// 削除処理の引数（コンテンツお気に入り情報）が想定通りか確認する
